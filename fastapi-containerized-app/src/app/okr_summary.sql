SELECT * FROM (SELECT 'PM Initiative Investment' as investment_category,
	a.normalized_sprint,
	count(a.issue_key) as num_jiras,
	sum(a.sum_story_points) as story_points 
FROM public.view_dedup_investment_flags a
WHERE a.sprint_name LIKE ANY (ARRAY['%2025.01%', '%2025.02%', '%2025.03%'])
	AND a.iris is null
...
WHERE a.sprint_name LIKE ANY (ARRAY['%2025.01%', '%2025.02%', '%2025.03%'])
	AND a.iris = true
GROUP BY a.normalized_sprint) a
