...
# --- Execution ---
echo "---" >> "$LOG_FILE"
echo "[$TIMESTAMP] Cron job started: Triggering /jobs/data-quality-report endpoint..." >> "$LOG_FILE"

# Execute the curl command to trigger the API.
/usr/bin/curl -s -X POST http://127.0.0.1:8000/jobs/data-quality-report >> "$LOG_FILE" 2>&1
