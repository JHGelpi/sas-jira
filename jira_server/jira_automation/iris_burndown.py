...
def get_epic_status_info(epic_key: str) -> tuple[bool, str]:
    """Return (is_closed, status_name) for the IRIS epic.
...
def collect_issue_keys_for_epic(epic_key: str, max_depth: int = MAX_DEPTH_DEFAULT) -> List:
    """
    Return the set of issues to count toward IRIS burndown for a given epic, defined as:
...
def upsert_burndown_row(run_dt: date, epic_key: str, bug: float, story: float, task_research: float, total: float) -> None:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO public.tbl_iris_burndown (run_date, epic_key, bug_points, story_points, task_research_points, total_points)
                VALUES (%s, %s, %s, %s, %s, %s)
...
def run_for_all_iris_epics(run_dt: date | None = None) -> Dict[str, Tuple[float, float, float, float]]:
    ...
    for epic in epic_keys:
        try:
            ...
            # Write/overwrite the HTML chart file for this epic (unless IRIS_SKIP_HTML is set)
            try:
                load_dotenv()
                skip_html = os.getenv("IRIS_SKIP_HTML", "").strip().lower() in {"1","true","yes","y"}
                if not skip_html:
...
def write_plot_html(epic_key: str, out_dir: str | None = None) -> str:
    ...
    path = os.path.join(base_dir, f"IRIS_{epic_key}_burndown.html")
